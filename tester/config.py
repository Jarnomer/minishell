...
class TestResult:
    ...
    stderr_match: bool = False  # Often just presence/absence
    exit_code_match: bool = False
    outfile_match: bool = True  # For redirection tests
    ...
    def passed(self) -> bool:
        base_pass = (
            self.exit_code_match and 
            self.stdout_match and 
            self.outfile_match and
            not self.minishell.crashed and
            self.zombie_count == 0
        )
        if self.valgrind:
