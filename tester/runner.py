...
from config import Config, Colors, TestCase, TestResult, ExecutionResult, ValgrindResult
...
class TestRunner:
    ...
    def _parse_valgrind_output(self, output: str) -> ValgrindResult:
        ...
        result.has_leaks = result.definitely_lost > 0 or result.indirectly_lost > 0
    ...
    def run_test(self, test: TestCase) -> TestResult:
        ...
        if is_syntax_test:
            ...
            stdout_match, stderr_match = self.compare_outputs(
                ms_result, ref_result, test=test, check_stderr=test.expect_error
            )
    ...
    def _compare_syntax_error(
        self, ms_result: ExecutionResult, ref_result: ExecutionResult, test: TestCase
    ) -> tuple[bool, bool]:
...
class TestPrinter:
    ...
    def print_header(self, title: str):
        ...
        print(
            f"{Colors.PURPLE}|{' ' * pad}{Colors.BOLD_GREEN}{title}{Colors.RESET}"
    ...
    def print_result(self, result: TestResult, test_num: int):
        ...
        if result.passed:
            print(
                f"{Colors.BOLD_PURPLE}TEST {test_num:3d}: ✅ {Colors.RESET}"
    ...
    def _print_verbose_failure(self, result: TestResult):
        ...
        if result.minishell.crashed:
            sig_name = (
                signal.Signals(result.minishell.signal).name
                if result.minishell.signal
                else "UNKNOWN"
            )
    ...
    def print_summary(self, summary: dict, test_count: int):
        ...
        print(
            f"{Colors.BOLD_BLUE}Tests passed:{Colors.RESET} {Colors.ok(str(summary['passed']))}"
        )
        print(
            f"{Colors.BOLD_BLUE}Tests failed:{Colors.RESET} {Colors.ko(str(summary['failed']))}"
        )
        ...
        if summary["by_category"]:
            ...
            for cat, counts in sorted(summary["by_category"].items()):
                ...
                print(
                    f"  {status} {cat}: {Colors.ok(str(counts['passed']))} passed, "
...
class TestLogger:
    ...
    def log_result(self, result: TestResult):
        ...
        with open(self.log_path, "a") as f:
            ...
            f.write(
                f"Exit codes: minishell={result.minishell.exit_code} "
